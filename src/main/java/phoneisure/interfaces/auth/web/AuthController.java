...
import phoneisure.application.auth.IAuthAppService;
    ...
    public ModelAndView login(@Valid @ModelAttribute("command") LoginCommand command, BindingResult bindingResult,
        ...
        if (flag) {
            try {
                ...
                if (subject.hasRole("admin") || subject.hasRole("agent") || subject.hasRole("picc") || subject.hasRole("auth")) {

//                    CoreHttpUtils.writeCookie(command, request, response);
                    //判断是不是省级代理
                    if (subject.hasRole("agent")) {
                        MerchantRepresentation agent = merchantAppService.searchByID(user.getId());
                        if (null == agent.getAgent() || null == agent.getAgent().getAgent()) {
